            ...
            <tbody class="divide-y divide-slate-100">
                @foreach ($ventas as $v)
                    <tr class="hover:bg-slate-50 transition-colors">
                        <td class="px-4 py-3 text-sm text-slate-600">{{ $v->fecha_emision }}</td>
                        <td class="px-4 py-3 text-sm font-mono text-slate-700">{{ $v->tipo_comprobante }} {{ $v->serie }}-{{ $v->numero }}</td>
                        <td class="px-4 py-3 text-sm text-slate-800">{{ $v->tercero->razon_social }}</td>
                        <td class="px-4 py-3 text-sm text-right text-slate-700">{{ number_format($v->base_imponible, 2) }}</td>
                        <td class="px-4 py-3 text-sm text-right text-slate-500">{{ number_format($v->igv, 2) }}</td>
                        <td class="px-4 py-3 text-sm text-right font-semibold text-slate-800">{{ number_format($v->total, 2) }}</td>
                        <td class="px-4 py-3">
                            @if ($v->asiento)
                                <a href="{{ route('empresas.asientos.show', [$empresa, $v->asiento]) }}"
                        ...
                        <td class="px-4 py-3 text-right">
                            @if (in_array($rolActual ?? null, ['administrador', 'contador']))
                                <form action="{{ route('empresas.ventas.destroy', [$empresa, $v]) }}" method="POST"
                                      onsubmit="return confirm('¿Eliminar esta venta y su asiento?')">
                                    @csrf @method('DELETE')

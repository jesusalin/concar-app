...
@section('content')
    <h1 class="text-2xl font-bold mb-4">Nueva venta — {{ $empresa->razon_social }}</h1>

    <form action="{{ route('empresas.ventas.store', $empresa) }}" method="POST" class="bg-white p-6 rounded shadow space-y-4 max-w-xl">
        @csrf
        <div>
            ...
            <select name="tercero_id" class="w-full border rounded p-2" required>
                ...
                @foreach ($clientes as $c)
                    <option value="{{ $c->id }}">{{ $c->numero_documento }} — {{ $c->razon_social }}</option>
                @endforeach
            </select>
            @if ($clientes->isEmpty())
                <p class="text-sm text-red-600 mt-1">No tienes clientes registrados. <a href="{{ route('empresas.terceros.create', $empresa) }}" class="underline">Crear uno</a>.</p>
            @endif
        </div>
